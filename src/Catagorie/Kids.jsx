...
export const Men = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      axios
        .get("http://localhost:8080/file/findbycatogries/Kids")
        .then((res) => {
          ...
          <div className='row'>
            {Array.isArray(categoryData) && categoryData.map((post) => (
              <div className="card m-2 col-12 col-sm-6 col-md-4 col-lg-3" key={post.id}>
                <img src={`http://localhost:8080/uploads/${post.image}`} style={{ height: "200px", width: "100%" }} className="card-img-top" alt='beerimage' />
                <div className='card-body'>
                 <h5 className='card-title'>{post.name}</h5>
                  <p>Description: <b style={{ fontSize: "larger" }}>{post.description}</b></p>
                  <p>Price: <b style={{ fontSize: "larger" }}>{post.price}</b></p>  
                  <button onClick={() => CartSend( post )} ><Link to='/Cart' style={{textDecoration:"none"}}>Add to Cart</Link></button>
                </div>
              </div>
            ))}
          </div>
